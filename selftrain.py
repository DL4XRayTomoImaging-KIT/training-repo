...
@cfg_ut('model', 'initializing models')
def get_models(**models_dict):
    
    models = {}
    for model_type, model_kwargs in models_dict.items():
      network_type = model_kwargs.pop('network_type')
      model = getattr(src.models, network_type)(**model_kwargs)
      models[model_type] = model
      trainable_parameters = sum(p.numel() for p in model.parameters())
      print(f'trainable parameters in {model_type} model: {trainable_parameters}')
    return models
...
def get_runner(runner_name='KDRunner', runner_kwargs=None):
    ...
    return getattr(src.runners, runner_name)()
...
def overall_training(cfg : DictConfig) -> None:
    ...
    if excluded:
        ...
        model = get_models(cfg=cfg)
        # load_weights(model, cfg=cfg)
        load_weights(model['teacher'], cfg=cfg)
        load_weights(model['student'], cfg=cfg)
        loaders = get_loaders(cfg=cfg, seed=seed)
        # optimizer = get_optimizer(model, cfg=cfg)
        optimizer = get_optimizer(model['student'], cfg=cfg)        
